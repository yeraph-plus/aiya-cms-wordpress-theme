<?php
//预定义数据
$items = aya_get_breadcrumb();

if ($items === false) {
    return;
}
?>
<div class="breadcrumbs text-sm" aria-label="Breadcrumb">
    <ul itemscope itemtype="https://schema.org/BreadcrumbList">
        <?php foreach ($items as $i => $item): ?>
            <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-item">
                <?php if ($item['url'] && $i < count($items) - 1): ?>
                    <a href="<?php echo esc_url($item['url']); ?>" itemprop="item" class="text-base-content/70 hover:text-primary transition-colors">
                        <?php if ($i === 0): ?>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
                            </svg>
                        <?php endif; ?>
                        <span itemprop="name"><?php echo esc_html($item['label']); ?></span>
                    </a>
                <?php else: ?>
                    <span itemprop="name" class="text-base-content"><?php echo esc_html($item['label']); ?></span>
                <?php endif; ?>
                <meta itemprop="position" content="<?php echo $i + 1; ?>" />
            </li>
        <?php endforeach; ?>
    </ul>
</div>