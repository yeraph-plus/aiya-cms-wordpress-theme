...
{
    if (!aya_opt('site_single_author_panel', 'layout')) return;
...
{
    if (!aya_opt('site_single_prev_next_post', 'layout')) return;
...
{
    if (!aya_opt('site_single_outdated_tip', 'layout')) return;
...
function aya_single_related_more()
{
    ...
    return '';
    //获取主题设置
    if (!aya_opt('site_single_related_post', 'layout') == false) return;
...
{
    //获取主题设置
    $dis_text = aya_opt('site_single_dis_claimer', 'layout');
