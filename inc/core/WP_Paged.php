...
if (!class_exists('AYA_WP_Paged_Object')) {
    ...
    function aya_get_pagination_html($paged = array())
    {
        ...
        $current = isset($paged['current']) ? (int) $paged['current'] : 1;
        $total = isset($paged['total']) ? (int) $paged['total'] : 1;

        $html = '<nav role="navigation" aria-label="Pagination" class="my-4 flex flex-col items-center gap-4">';
        $html .= '<div class="mx-auto flex w-full justify-center">';
        $html .= '<div class="flex flex-row items-center gap-1">';

        if (!empty($prev_link['url'])) {
            $html .= '<a href="' . esc_url((string) $prev_link['url']) . '" rel="prev" aria-label="' . esc_attr__('Go previous page', 'AIYA') . '" class="inline-flex h-9 items-center rounded-md px-3 text-sm hover:bg-accent">' . esc_html((string) ($prev_link['text'] ?? __('上一页', 'AIYA'))) . '</a>';
        } else {
            $html .= '<span class="inline-flex h-9 items-center rounded-md px-3 text-sm opacity-50">' . esc_html__('上一页', 'AIYA') . '</span>';
        }

        $html .= '<span class="inline-flex h-9 items-center rounded-md px-3 text-sm text-muted-foreground">';
        $html .= esc_html(sprintf(__('第 %s 页，共 %s 页', 'AIYA'), $current, $total));
        $html .= '</span>';

        if (!empty($next_link['url'])) {
            $html .= '<a href="' . esc_url((string) $next_link['url']) . '" rel="next" aria-label="' . esc_attr__('Go next page', 'AIYA') . '" class="inline-flex h-9 items-center rounded-md px-3 text-sm hover:bg-accent">' . esc_html((string) ($next_link['text'] ?? __('下一页', 'AIYA'))) . '</a>';
        } else {
            $html .= '<span class="inline-flex h-9 items-center rounded-md px-3 text-sm opacity-50">' . esc_html__('下一页', 'AIYA') . '</span>';
        }
