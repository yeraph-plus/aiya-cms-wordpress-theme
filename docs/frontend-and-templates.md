...
- `src/runtime/badge-slots.tsx`：扫描 `.badge-slot`，根据 `data-badge-variant` 或 `data-badge-alias` 注入徽章样式和内置图标
- `src/runtime/alert-slots.tsx`：扫描 `.alert-slot`，根据 PHP 占位标签替换为 Alert 组件的 HTML
...
- `src/entrypoints/standalone.ts`：只注册独立页入口，不再做文章内容增强
...
- `templates/fragments/*.php`：可复用片段模板，例如 `breadcrumb`、`loop-grid`、`content-detail`
...
- 根据 `usePreferencesStore()` 中的布局偏好切换 `grid / list`
