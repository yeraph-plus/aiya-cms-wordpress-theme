 ...
 * 判断是否应该忽略点击事件
 */
function shouldIgnoreClick(e: MouseEvent, link: HTMLAnchorElement, url: URL): boolean {
...
/**
 * 设置链接拦截（非 React Hook 版本）
 * 返回清理函数
 */
export function setupLinkInterception(navigate: (url: string) => void): () => void {
...
export function usePJAXFetch() {
    ...
    const fetchPageHook = useCallback(async (url: string): Promise<FetchResult> => {
        requestId.current++;
    ...
    return { fetchPage: fetchPageHook, abort };
}

export function useLinkInterception(navigate: (url: string) => void) {
    useEffect(() => {
        return setupLinkInterception(navigate);
    }, [navigate]);
}
